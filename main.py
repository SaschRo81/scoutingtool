...
def generate_custom_sections_html(offense_df, defense_df, about_df):
    ...
    def make_section(title, df):
        if df.empty:
            return ""
        section_html = (
            f"<h3 style='border-bottom: 2px solid #333; margin-bottom:10px; "
            f"font-family: Arial, sans-serif;'>{title}</h3>"
        )
        section_html += (
            "<table style='width:100%; border-collapse:collapse; "
            "font-family:Arial, sans-serif; font-size:12px; margin-bottom:20px;'>"
        )
...
if not st.session_state.print_mode:
    ...
    if st.button(f"2. Kader von {scout_target} laden", type="primary"):
        ...
        api_team = (
            f"https://api-s.dbbl.scb.world/seasons/{SEASON_ID}/team-statistics"
            f"?displayType=MAIN_ROUND&teamId={target_team_id}"
        )
        ...
        try:
            ...
            if raw:
                ...
                def get_v(k):
                    return (
                        pd.to_numeric(df[final_cols[k]], errors="coerce").fillna(0)
                        if k in final_cols
        ...
        if len(selected_indices) > 0:
            ...
            with st.form("scouting_form"):
                ...
                for _, row in selection.iterrows():
                    ...
                    l1 = c1.text_input(
                        "L1", value=l1v, key=f"l1_{pid}", label_visibility="collapsed"
                    )
                    l2 = c1.text_input(
                        "L2", value=l2v, key=f"l2_{pid}", label_visibility="collapsed"
                    )
                    l3 = c1.text_input(
                        "L3", value=l3v, key=f"l3_{pid}", label_visibility="collapsed"
                    )
                    l4 = c1.text_input(
                        "L4", value=l4v, key=f"l4_{pid}", label_visibility="collapsed"
                    )
                    r1 = c2.text_input(
                        "R1", value=r1v, key=f"r1_{pid}", label_visibility="collapsed"
                    )
                    r2 = c2.text_input(
                        "R2", value=r2v, key=f"r2_{pid}", label_visibility="collapsed"
                    )
                    r3 = c2.text_input(
                        "R3", value=r3v, key=f"r3_{pid}", label_visibility="collapsed"
                    )
                    r4 = c2.text_input(
                        "R4", value=r4v, key=f"r4_{pid}", label_visibility="collapsed"
                    )
                ...
                uploaded_files = st.file_uploader(
                    "Upload", accept_multiple_files=True, type=["png", "jpg", "jpeg"]
                )

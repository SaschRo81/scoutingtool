...
def generate_custom_sections_html(offense_df, defense_df, about_df):
    ...
    def make_section(title, df):
        ...
        for _, r in df.iterrows():
            ...
            section_html += (
                f"<tr><td style='width:30%; border:1px solid #ccc; padding:4px; font-weight:bold; vertical-align:top;'>{c1}</td>"
                f"<td style='border:1px solid #ccc; padding:4px; vertical-align:top;'>{c2}</td></tr>"
            )
        section_html += "</table>"
        return section_html

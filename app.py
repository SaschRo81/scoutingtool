...
def render_home():
    ...
    with c2:
        ...
        if st.button("📊 Teamvergleich"):
            go_comparison()
            st.rerun()
            
        if st.button("📝 Scouting Report"):
            go_scouting()
            st.rerun()
            
        if st.button("🎥 Spielnachbereitung"):
            go_analysis()
            st.rerun()
...
def render_analysis_page():
    ...
    st.info("🚧 Dieser Bereich ist noch im Aufbau.")
    st.write("Geplante Funktionen:")
    st.markdown("- Auswahl der Partie")
    st.markdown("- Boxscore Import")
    st.markdown("- Play-by-Play Analyse")
    st.markdown("- Automatischer Spielbericht")

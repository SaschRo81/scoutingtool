...
if not st.session_state.print_mode:
    ...
    with c2:
        home_name = st.selectbox("Heim:", list(teams_filtered.keys()), 0)
        home_id = teams_filtered[home_name]
        # Logo Cache
    ...
    with c3:
        guest_name = st.selectbox("Gast:", list(teams_filtered.keys()), 1)
        guest_id = teams_filtered[guest_name]
    ...
    st.write("---")
    target = st.radio("Target:", ["Gastteam (Gegner)", "Heimteam"], horizontal=True)
    ...
    # 2. DATEN LADEN (LOGIK VERBESSERT)
    st.divider()
    
    # Prüfen ob Daten da sind UND ob sie zum aktuellen Team passen
    ...
    if st.button(f"2. Kader von {target} laden", type="primary") or data_ready:
        # Nur laden, wenn noch nicht da oder Team gewechselt
        if not data_ready:
            with st.spinner("Lade API..."):
                df, ts = fetch_team_data(tid, SEASON_ID)
                if df is not None:
                    st.session_state.roster_df = df
                    st.session_state.team_stats = ts
                    st.session_state.current_tid = tid # Merken welches Team
                else:
                    st.error("Fehler API")
    ...
    # 3. EDITIEREN (Wird nur angezeigt, wenn Daten da sind)
    if st.session_state.roster_df is not None:
        ...
        # WICHTIG: key="player_selector" hält den State fest!
        ...
        # Indizes der ausgewählten Spieler

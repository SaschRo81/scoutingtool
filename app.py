...
from src.api import (
    get_team_data_no_cache, get_player_metadata_cached, fetch_schedule, 
...
def render_team_stats_page():
    if st.session_state.stats_team_id:
        ...
        with st.spinner("Lade Team Statistiken..."):
            df, ts = get_team_data_no_cache(tid, CURRENT_SEASON_ID)
...
def render_comparison_page():
    ...
    if st.button("Vergleich starten", type="primary"):
        with st.spinner("Lade Daten..."):
            _, ts_h = get_team_data_no_cache(h_id, CURRENT_SEASON_ID)
            _, ts_g = get_team_data_no_cache(g_id, CURRENT_SEASON_ID)
...
def render_player_comparison_page():
    ...
    with c1:
        ...
        df1, _ = get_team_data_no_cache(tid1, CURRENT_SEASON_ID)
    ...
    with c3:
        ...
        df2, _ = get_team_data_no_cache(tid2, CURRENT_SEASON_ID)
...
def render_prep_page():
    ...
    if st.button("Vorbereitung starten", type="primary"):
        with st.spinner("Lade Daten..."):
            df, _ = get_team_data_no_cache(opp_id, CURRENT_SEASON_ID)

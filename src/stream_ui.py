...
.potg-stat-box { display: flex; justify-content: center; gap: 15px; margin-top: 25px; background: rgba(255,255,255,0.1); padding: 15px; border-radius: 10px; }
    ...
    for team_key in ["homeTeam", "guestTeam"]:
        for p in box.get(team_key, {}).get("playerStats", []):
            try:
                ...
                players.append({"id": str(p.get("seasonPlayer", {}).get("id")), "name": f"{p.get('seasonPlayer', {}).get('firstName','')} {p.get('seasonPlayer', {}).get('lastName','')}", "nr": p.get('seasonPlayer', {}).get('shirtNumber', ''), "eff": eff, "pts": int(p.get("points", 0)), "min": min_str, "reb": reb})
            except: pass
    if players:
        ...
        # HTML sauber konkatenieren
        html = "<div class='potg-card'>"
        html += "<h2 style='color:#ff6600; margin:0 0 15px 0; font-size:24px;'>PLAYER OF THE GAME</h2>"

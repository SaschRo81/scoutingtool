...
def render_obs_standings():
    ...
    if not df.empty:
        ...
        html = f"<div style='position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); width:1400px; background:white; border-radius:15px; border:3px solid #00338d; overflow:hidden; box-shadow:0 0 50px rgba(0,0,0,0.8);'>"
...
def render_obs_comparison():
    ...
    html = f"<div style='background:white; width:1500px; margin:50px auto; border-radius:15px; overflow:hidden; box-shadow:0 20px 60px rgba(0,0,0,0.8); font-family:sans-serif;'>"
    html += f"<table style='width:100%; border-collapse:collapse;'>"
    html += f"<tr><th style='background:#FFD700; color:black; padding:25px; font-size:32px; font-weight:900; width:40%; text-align:center;'>{hname}</th>"
    ...
    for label, key in metrics:
        ...
        html += f"<td style='padding:15px; {c_g}'>{f_g}</td></tr>"
...
def render_obs_final_banner():
    ...
    if h_logo: html += f"<img src='{h_logo}' style='height:90px;'>"
    else: html += "<div></div>"
    ...
    if g_logo: html += f"<img src='{g_logo}' style='height:90px;'>"
    else: html += "<div></div>"
    html += "</div></div>"
    st.markdown(html, unsafe_allow_html=True)

...
def render_live_view(box):
    ...
    st.markdown(f"""<div style='text-align:center;background:#222;color:#fff;padding:10px;border-radius:10px;margin-bottom:15px;'><div>{h_name} vs {g_name}</div><div style='font-size:2.5em;font-weight:bold;'>{res.get('homeTeamFinalScore', 0)} : {res.get('guestTeamFinalScore', 0)}</div><div style='color:#ccc;'>Q{res.get('period', '-')} | {convert_elapsed_to_remaining(box.get('gameTime', ''), res.get('period', ''))}</div></div>""", unsafe_allow_html=True)
    c1, c2 = st.columns([1, 1])
    with c1:

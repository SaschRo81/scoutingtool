...
def render_full_play_by_play(box, height=600):
    ...
    # Wenn im Live-Modus (height=400), sortiere neueste zuerst
    if height == 400:
        df = df.iloc[::-1]
...
def render_boxscore_table_pro(player_stats, team_stats_official, team_name, coach_name="-"):
    ...
    for p in player_stats:
        ...
        s_2p = f"{m2}/{a2} ({int(p2)}%)" if a2 and sec > 0 else ""
        s_3p = f"{m3}/{a3} ({int(p3)}%)" if a3 and sec > 0 else ""
        s_fg = f"{mfg}/{afg} ({int(pfg)}%)" if sec > 0 else ""
        s_ft = f"{mft}/{aft} ({int(pft)}%)" if aft and sec > 0 else ""
...
def render_game_top_performers(box):
    h_data = box.get("homeTeam", {}); g_data = box.get("guestTeam", {})
...
def render_charts_and_stats(box):
    h = box.get("homeTeam", {}).get("gameStat", {}); g = box.get("guestTeam", {}).get("gameStat", {})
    h_name = get_team_name(box.get("homeTeam", {}), "Heim"); g_name = get_team_name(box.get("guestTeam", {}), "Gast")
    actions = box.get("actions", [])
    hid = str(box.get("homeTeam", {}).get("seasonTeam", {}).get("seasonTeamId", "0"))
    gid = str(box.get("guestTeam", {}).get("seasonTeam", {}).get("seasonTeamId", "0"))
    cs = calculate_advanced_stats_from_actions(actions, hid, gid)
...
def generate_game_summary(box):
    ...
    winner = h_name if s_h > s_g else g_name; diff = abs(s_h - s_g)
    ...
    q_h = [res.get(f"homeTeamQ{i}Score", 0) for i in range(1, 5)]; q_g = [res.get(f"guestTeamQ{i}Score", 0) for i in range(1, 5)]
    ...
    def get_best(p_list, key):
        ...
        s = sorted(p_list, key=lambda x: safe_int(x.get(key)), reverse=True); p = s[0]
        return p.get("seasonPlayer", {}).get("lastName", "Unknown"), safe_int(p.get(key))
...
def generate_complex_ai_prompt(box):
    ...
    def get_stats_str(team_data):
        s = team_data.get("gameStat", {}); p_list = team_data.get("playerStats", [])
    ...
    prompt = f"""
Erstelle 3 journalistische Spielberichte (Heim-Sicht, Neutral, Gäste-Sicht) basierend auf diesen Daten:
...
def run_openai_generation(api_key, prompt):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o", 
            messages=[{"role": "system", "content": "Du bist ein Sportjournalist."}, {"role": "user", "content": prompt}],
            temperature=0.7,
        )

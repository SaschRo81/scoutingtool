...
def prepare_ai_scouting_context(team_name, detailed_games, team_id):
    ...
    for g in detailed_games:
        ...
        # Player Mapping
        p_map = {str(p.get("seasonPlayer",{}).get("id")): f"#{p.get('seasonPlayer',{}).get('shirtNumber')} {p.get('seasonPlayer',{}).get('lastName')}" 
                 for p in my_team.get("playerStats", [])}
        
        # Starting 5
        starters = [p_map.get(str(p.get("seasonPlayer",{}).get("id"))) for p in my_team.get("playerStats", []) if p.get("isStartingFive")]
        ...
        for act in actions[:12]:
            ...
            prefix = f"WIR ({p_map.get(actor_id, 'Team')})" if actor_id in p_map else "GEGNER"
        ...
        for i, act in enumerate(actions):
            if "TIMEOUT" in str(act.get("type")).upper():
                ...
                for j in range(1, 4):
                    if i+j < len(actions):
                        na = actions[i+j]
                        na_id = str(na.get("seasonPlayerId"))
...
def calculate_real_lineups(team_id, detailed_games):
    ...
    for box in detailed_games:
        ...
        for act in actions:
            ...
            try:
                m, s = map(int, raw_time.split(':')[-2:])
                curr_secs = (act.get("period", 1)-1)*600 + (m*60 + s)
            except: curr_secs = last_secs
            
            # Auto-Korrektur falls Spieler Aktion macht aber nicht im Set ist
...
def render_team_analysis_dashboard(team_id, team_name):
    ...
    with c2:
        ...
        st.caption("PBP Saison-Analyse 2025")
...
# --- BENÖTIGTE FUNKTIONEN FÜR APP.PY ---
...
def render_charts_and_stats(box): st.info("Charts & erweiterte Team-Stats")
def render_game_top_performers(box): st.info("Top Performer des Spiels")
def generate_game_summary(box): return "Kurze Zusammenfassung des Spielverlaufs..."
def generate_complex_ai_prompt(box): return "ChatGPT Prompt Kontext..."
def run_openai_generation(api_key, prompt): return "KI Service ist bereit."

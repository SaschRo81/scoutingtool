...
def render_game_header(box, details=None):
    ...
    period = "Ende"
    time_str = ""
    status = details.get("status") if details else box.get("status", "SCHEDULED")
    
    if status == "RUNNING":
        p_raw = details.get("period", box.get("period", 0))
        time_raw = details.get("gameTime", box.get("gameTime", "00:00"))
        period = f"Q{p_raw}"
        time_str = f" | {time_raw}"
    ...
    with c_score:
        ...
        st.markdown(f"""<div style='text-align:center; background:#f8f9fa; padding:15px; border-radius:10px; border:1px solid #ddd;'><div style='font-size:16px; color:#666; font-weight:bold; text-transform:uppercase;'>{period}{time_str}</div><div style='font-size:48px; font-weight:900; color:#333; line-height:1.2;'>{h_s} : {g_s}</div></div>""", unsafe_allow_html=True)
...
def render_boxscore_table_pro(player_stats, game_stat, team_name, coach):
    ...
    if coach: st.caption(f"Head Coach: {coach}")
    ...
    if game_stat:
        rows.append({"#": "", "Name": "TEAM TOTAL", "Min": "", "PTS": game_stat.get("points", 0), "REB": game_stat.get("totalRebounds", 0), "AST": game_stat.get("assists", 0), "STL": game_stat.get("steals", 0), "TO": game_stat.get("turnovers", 0), "BLK": game_stat.get("blocks", 0), "PF": game_stat.get("foulsCommitted", 0), "EFF": game_stat.get("efficiency", 0), "+/-": 0, "FG": "", "3P": "", "FT": ""})

    df = pd.DataFrame(rows)
    df["#"] = df["#"].astype(str); df["Name"] = df["Name"].astype(str)
    cols = ["#", "Name", "PTS", "REB", "AST", "EFF", "Min", "FG", "3P", "FT", "TO", "STL", "BLK", "PF", "+/-"]
    st.dataframe(df[cols], hide_index=True, use_container_width=True, column_config={"#": st.column_config.TextColumn("#", width="small"), "Name": st.column_config.TextColumn("Name", width="medium")})
...
def render_full_play_by_play(box):
    ...
    for a in actions_rev:
        ...
        st.markdown(f"<div style='border-bottom:1px solid #eee; padding:8px 0; display:flex; align-items:center;'><div style='width:60px; font-weight:bold; color:#666;'>Q{q} {time}</div><div style='width:30px; text-align:center;'>{icon}</div><div style='flex-grow:1;'>{desc}</div><div>{score_badge}</div></div>", unsafe_allow_html=True)
    ...
    if actions:
        ...
        for a in actions:
            if a.get("homeTeamPoints") is not None:
                ...
                try:
                    mins, secs = map(int, gt.split(":")); q = a.get("period", 1)
...
def render_prep_dashboard(opp_id, opp_name, df_roster, schedule, metadata_callback):
    ...
    with c1:
        ...
        from src.api import fetch_last_n_games_complete
    ...
    with c2:
        ...
        if df_roster is not None:
             cols = [c for c in ["NR", "NAME_FULL", "PPG", "GP"] if c in df_roster.columns]
             st.dataframe(df_roster[cols], hide_index=True)
...
def render_team_analysis_dashboard(tid, tname): st.title(f"Deep Dive: {tname}"); st.write("Erweiterte Statistiken (Clutch, Lineups) kommen hier.")

...
def render_boxscore_table_pro(player_stats, team_stats_official, team_name, coach_name="-"):
    ...
    for p in player_stats:
        ...
        # Fallback wenn FG nicht geliefert
    ...
    # 2. Team/Coach Zeile berechnen (Differenz zu Official Stats)
    if team_stats_official:
        ...
        # Wenn es Team-Stats gibt, Zeile hinzufügen
        if (tm_or+tm_dr+tm_to+tm_pf+tm_pts) != 0:
            data.append({
                "#": "", "Name": "Team / Coach", "Min": "", 
                "PTS": tm_pts if tm_pts!=0 else "", 
                "2P": "", "3P": "", "FT": "",
                "OR": tm_or, "DR": tm_dr, "TR": tm_tr,
                "AS": tm_as, "TO": tm_to, "ST": tm_st, "BS": tm_bs, "PF": tm_pf, "+/-": ""
            })
            
            # Summen für TOTALS anpassen (damit TOTALS = Official Stats sind)

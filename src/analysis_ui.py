...
def render_live_view(box):
    ...
    t_rem, t_orig = get_time_info(box.get('gameTime') or (actions[-1].get('gameTime') if actions else None), period)
    p_int = safe_int(period)
    p_str = (f"OT{p_int-4}" if p_int > 4 else f"{p_int}Q")  # 1Q/2Q/...
    ...
    with t1:
        ...
        with c1:
            ...
            if not dfh.empty:
                st.dataframe(dfh.style.apply(style_live, axis=1), hide_index=True, use_container_width=True,
                             height=(len(dfh)+1)*35+3)
        with c2:
            st.markdown(f"### {g_name}")
            dfg = create_live_boxscore_df(g_data)
            if not dfg.empty:
                st.dataframe(dfg.style.apply(style_live, axis=1), hide_index=True, use_container_width=True,

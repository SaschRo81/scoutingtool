...
def render_prep_dashboard(team_id, team_name, df_roster, last_games, metadata_callback=None):
    # DIESE FUNKTION WURDE VERBESSERT, UM ROHE DATEN ZU VERMEIDEN
    st.subheader(f"Gegner-Analyse: {team_name}")
    c1, c2 = st.columns([2, 1])
    
    with c1:
        st.markdown("#### Top 4 Spieler (nach PPG)")
        if df_roster is not None and not df_roster.empty:
            # Sortieren und sicherstellen, dass wir Daten haben
            if "PPG" in df_roster.columns:
                top4 = df_roster.sort_values(by="PPG", ascending=False).head(4)
            else:
                top4 = df_roster.head(4)

            for _, row in top4.iterrows():
                # Daten extrahieren und Fallbacks nutzen
                p_name = row.get('NAME_FULL', 'Unbekannt')
                p_nr = row.get('NR', '-')
                p_ppg = row.get('PPG', 0)
                p_fg = row.get('FG%', 0)
                p_3p = row.get('3PCT', 0)
                ...
                with st.container(border=True):
                    ...
                    with col_img:
                        # Versuchen Bild zu laden via Callback
                        img_url = None
                    ...
                    with col_stats:
                        st.markdown(f"**#{p_nr} {p_name}**")
                        # Stats Zeile
                        st.markdown(f"**{p_ppg} PPG** | FG: {p_fg}% | 3P: {p_3p}%")
        else: 
            st.warning("Keine Kaderdaten für diesen Gegner verfügbar.")

    with c2:
        st.markdown("#### Letzte Spiele")
        if last_games:
            # Iterieren statt Raw Dump!
            for g in last_games:
                # Datum parsen für bessere Anzeige
                d_str = g.get('date', '-')
                ...
                # Logik: Wer war Gegner? (Wir nehmen an, wir bereiten uns auf 'team_name' vor)
                # Wir müssen raten wer wer ist basierend auf dem String, oder es ist egal
                ...
                with st.container(border=True):
                    st.caption(f"{d_str} | {loc}")
                    st.markdown(f"**vs {opponent}**")
                    st.markdown(f"Ergebnis: **{score}**")
        else:
...
def analyze_scouting_data(team_id, detailed_games):
    ...
    for box in detailed_games:
        # Verbesserte Heim/Gast Erkennung mit Meta-Daten aus api.py
        ...
        # ATO Logik (benötigt korrekte IDs)
        # Wir suchen nach der ID, die das Timeout genommen hat.
        # Wenn wir Home sind, suchen wir nach Home-Timeouts.
        home_tid = str(box.get("homeTeam", {}).get("seasonTeamId"))
        guest_tid = str(box.get("guestTeam", {}).get("seasonTeamId"))
        target_timeout_id = home_tid if is_home else guest_tid

        for i, act in enumerate(actions):
            if "TIMEOUT" in str(act.get("type")).upper() and str(act.get("seasonTeamId")) == target_timeout_id:
                ...
                for j in range(1, 6):
                    ...
                    # Punkte zählen, wenn das Team (wir) scored
                    if pts > 0 and act_tid == target_timeout_id: stats["ato_stats"]["points"] += pts; break
                    # Abbruch wenn Gegner scored oder wir TO machen
                    if (pts > 0 and act_tid != target_timeout_id) or (next_act.get("type") == "TURNOVER" and act_tid == target_timeout_id): break
...
def prepare_ai_scouting_context(team_name, detailed_games, team_id):
    ...
    for g in detailed_games:
        ...
        opp = g.get('meta_opponent', 'Gegner'); res = g.get('meta_result', 'N/A'); date_game = g.get('meta_date', 'Datum?')
        ...
        for act in reversed(actions):
            ...
            if str(act.get("seasonTeamId")) == my_season_id:
                pid = str(act.get("seasonPlayerId"))
                if pid and pid != "None": closers.add(player_map.get(pid, "Unbekannt"))
        if closers: context += f"Closing Lineup (Endphase): {', '.join(list(closers))}\n"
        ...
        for act in actions:
            if act.get("period") == 1:
                ...
                tid = str(act.get("seasonTeamId"))
                actor = "WIR" if tid == my_season_id else "GEGNER"
                pid = str(act.get("seasonPlayerId")); p_name = player_map.get(pid, "")
        ...
        for i, act in enumerate(actions):
            if "TIMEOUT" in str(act.get("type")).upper():
                ...
                if to_tid == my_season_id:
                    found_to = True
                    context += f"TIMEOUT (WIR) genommen.\n"
                    for j in range(1, 5):
                        if i+j < len(actions):
                            na = actions[i+j]; ntid = str(na.get("seasonTeamId"))
                            who_act = "WIR" if ntid == my_season_id else "GEGNER"
...
def render_team_analysis_dashboard(team_id, team_name):
    ...
    k2.metric("Start-Qualität (Q1)", f"{scout['start_stats']['avg_diff']:+.1f}", help="Durchschnittliche Punktedifferenz in den ersten 5 Minuten")

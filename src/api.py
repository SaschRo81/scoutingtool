...
def fetch_season_games(season_id):
    return fetch_games_from_recent()

# --- TREND & TABELLE ---
@st.cache_data(ttl=1800)
def fetch_league_standings(season_id, league_selection):
    """
    Holt Tabelle mit den gewünschten Spalten:
    Rank, Name, TotalGames, TotalVictories, TotalLosses, Diff.
    """
    urls = []
    ...
    for url in urls:
        try:
            ...
            if r.status_code == 200:
                ...
                table_data = []
                for entry in items:
                    # EXTRAHIERE GENAU DIE GEFORDERTEN FELDER
                    rank = entry.get("rank", 0)
                    name = entry.get("seasonTeam", {}).get("name", "Unknown")
                    games = entry.get("totalGames", 0)
                    wins = entry.get("totalVictories", 0)
                    losses = entry.get("totalLosses", 0)
                    # "awayGoalDifference" falls vorhanden, sonst pointsDifference
                    diff = entry.get("awayGoalDifference")
                    if diff is None:
                        diff = entry.get("pointsDifference", 0)

                    diff_str = f"+{diff}" if diff > 0 else str(diff)

                    table_data.append({
                        "Platz": rank, "Team": name, 
                        "Sp": games, "S": wins, "N": losses, 
                        "Diff": diff_str
                    })

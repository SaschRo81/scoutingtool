    ...
    for subdomain in ["api-s", "api-n"]:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                return {"img": img, "height": height, "pos": str(pos).replace("_", " "), "age": age, "nationality": nat}
        except: pass
    return {"img": "", "height": "-", "pos": "-", "age": "-", "nationality": "-"}

@st.cache_data(ttl=600)
def fetch_team_details_raw(team_id, season_id):
    urls = [f"https://api-1.dbbl.scb.world/teams/{team_id}/{season_id}", f"{get_base_url(team_id)}/teams/{team_id}/{season_id}"]
    for url in urls:
        try:
    ...
    try:
        ...
        if r_team.status_code == 200:
            ...
            if isinstance(td, dict) and td.get("gamesPlayed"):
                ...
                ts = {"ppg": (td.get("points",0)/gp), "tot": (td.get("totalRebounds",0)/gp), "as": (td.get("assists",0)/gp), "to": (td.get("turnovers",0)/gp), "st": (td.get("steals",0)/gp), "bs": (td.get("blocks",0)/gp), "pf": (td.get("foulsCommitted",0)/gp), "fgpct": (td.get("fieldGoalsMade",0)/td.get("fieldGoalsAttempted",1)*100), "3pct": (td.get("threePointShotsMade",0)/td.get("threePointShotsAttempted",1)*100), "ftpct": (td.get("freeThrowsMade",0)/td.get("freeThrowsAttempted",1)*100), "dr": (td.get("defensiveRebounds",0)/gp), "or": (td.get("offensiveRebounds",0)/gp), "2m": (td.get("fieldGoalsMade",0)-td.get("threePointShotsMade",0))/gp, "2a": (td.get("fieldGoalsAttempted",0)-td.get("threePointShotsAttempted",0))/gp, "3m": td.get("threePointShotsMade",0)/gp, "3a": td.get("threePointShotsAttempted",0)/gp, "ftm": td.get("freeThrowsMade",0)/gp, "fta": td.get("freeThrowsAttempted",0)/gp}
    except: pass
    try:
        ...
        if r_stats.status_code == 200:
            ...
            if p_list:
                df = pd.json_normalize(p_list); df.columns = [str(c).lower() for c in df.columns]
                c_id = next((c for c in df.columns if "id" in c), "id")
                df["PLAYER_ID"] = df[c_id].astype(str).str.replace(".0", "", regex=False)
                ...
                df["FG%"] = (pd.to_numeric(df["fieldgoalssuccesspercent"], errors="coerce") * 100).round(1).fillna(0)
                df["3PCT"] = (pd.to_numeric(df["threepointshotsuccesspercent"], errors="coerce") * 100).round(1).fillna(0)
                df["FTPCT"] = (pd.to_numeric(df["freethrowssuccesspercent"], errors="coerce") * 100).round(1).fillna(0)
...
def fetch_schedule(team_id, season_id):
    url = f"https://api-s.dbbl.scb.world/games?currentPage=1&seasonTeamId={team_id}&pageSize=100&seasonId={season_id}"
    try:
                ...
                for slot in ["past", "present", "future"]:
                    if slot in data and isinstance(data[slot], list):
                        for g in data[slot]:
                            ...
                            h_s = res.get("homeScore") if res.get("homeScore") is not None else res.get("homeTeamFinalScore")
                            g_s = res.get("guestScore") if res.get("guestScore") is not None else res.get("guestTeamFinalScore")
                            all_games.append({"id": game_id, "date": d_disp, "date_only": date_only, "home": g.get("homeTeam", {}).get("name", "Heim"), "guest": g.get("guestTeam", {}).get("name", "Gast"), "score": f"{h_s if h_s is not None else '-'}:{g_s if g_s is not None else '-'}", "status": g.get("status")})
        except: continue
    return all_games
            ...
            if r.status_code == 200:
                ...
                for entry in items:
                    ...
                    if str(st_obj.get("id")) == str(team_id) or str(st_obj.get("teamId")) == str(team_id):
                        return {"rank": entry.get("rank"), "totalGames": entry.get("totalGames"), "totalVictories": entry.get("totalVictories"), "totalLosses": entry.get("totalLosses"), "last10Victories": entry.get("last10Victories",0), "last10Losses": entry.get("last10Losses",0)}
        except: continue
    return None
